...
class S7BankDepositScreen extends StatefulWidget {
  const S7BankDepositScreen({super.key});

  @override
  State<S7BankDepositScreen> createState() => _S7BankDepositScreenState();
}

class _S7BankDepositScreenState extends State<S7BankDepositScreen> {
  DateTime fromDate = DateTime(2025, 1, 1);
  DateTime toDate = DateTime(2025, 1, 31);
  final List<BankEntry> data = BankEntry.getMockData();
            ...
            rows: data.map((entry) {
              return DataRow2(
                cells: [
                  ...
                  DataCell(Text(
                    entry.deposit > 0
                        ? NumberFormatter.formatCurrency(entry.deposit)
                        : '',
                    style: const TextStyle(color: Colors.green),
                  )),
                  DataCell(Text(
                    entry.withdrawal > 0
                        ? NumberFormatter.formatCurrency(entry.withdrawal)
                        : '',
